...
<body>
	<div class="container">
		<h1 class="display-4 animated slideInLeft">Ofertas</h1>
		<div>
		...
		<form action="<?php echo base_url();?>oferta_controller/accion" name="mostrar" method="POST">
			<table id="dtBasicExample" class="table table-striped table-bordered" cellspacing="0" width="100%">
				<thead class="thead-dark">
					<tr>
						<th class="th-sm">Modificar</th>
						<th class="th-sm">Monto</th>
						<th class="th-sm">Fecha de la oferta</th>
				...
				<tbody>
					<?php foreach($oferta as $ofer): ?>
						<tr>
							<td><input type="radio" name="editar" value="<?=$ofer->id_oferta ?>" required></td>
							<td><?=$ofer->monto ?></td>
							<td><?=$ofer->fecha_oferta ?></td>
							<td><?=$ofer->nombre.' '.$ofer->apellido ?></td>
						</tr>
					<?php endforeach; ?>
				</tbody>
			</table>
			<div>
				...
				<button class="btn btn-info">
					<a href="<?php echo base_url();?>oferta_controller/agregar_accion"><i class="fas fa-plus"></i>&nbsp;Agregar</a>
				</button>
				<!-- actualizar -->
				...
				<!-- Eliminar -->
				<button class="btn btn-info" type="submit">
					<a href="<?php echo base_url();?>oferta_controller/eliminar_a"><i class="fas fa-trash"></i>&nbsp;Eliminar</a>
				</button>
			</div>
		</form>
		<br>
